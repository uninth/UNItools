...
# 
#
#                                      Mon Mar 24 16:20:48 CET 2014
# Preample
# --------
# This script is the documentation for how iptraf 2.7.0 was downloaded,
...
# All configuration files:	/var/opt/UNItools/etc/iptraf
# All relevant documentation:	/var/opt/UNItools/docs/iptraf.2.7.0
# 
...
# cd /lan/ssi/shared/software/internal/UNItools/src/GaIA/pkgs/iptraf
# 
#  1	Download the package iptraf, and read the package documentation,
# 	usually README and INSTALL.
# 
# wget -N ftp://iptraf.seul.org/pub/iptraf/iptraf-2.7.0.tar.gz
# 
# /bin/gtar xvzf iptraf-2.7.0.tar.gz
# 
...
URL="ftp://iptraf.seul.org/pub/iptraf/iptraf-2.7.0.tar.gz"
PKG="iptraf"
VER="2.7.0"
...
VENDOR="ftp://iptraf.seul.org/pub/iptraf/iptraf-2.7.0.tar.gz"
...
install:
	test -d /var/opt/UNItools/log || mkdir -p /var/opt/UNItools/log
	cd src && echo n | $(MAKE) install clean
EOF
	...
	sed "
		s/^CFLAGS.*/CFLAGS	= -Wall -static #-O2 #-m486/g
		s%^TARGET.*%TARGET	= /var/opt/UNItools/bin%g
		s%^WORKDIR.*%WORKDIR	= /var/opt/UNItools/log/iptraf%g
		s%^LOGDIR.*%LOGDIR	= /var/opt/UNItools/log/iptraf%g" Makefile.dist > Makefile
	
	)

# 	If iptraf is made with automake and configure then
# 
 	pkg_conbat -n ./CFG
# 
...
# 	/var/opt/UNItools
# 	If iptraf requires something else to compile, please correct the
# 	section that writes to CFG.
# 	The new files are then moved to iptraf.2.7.0.install.tmp - which
# 	will be wiped first.
# 	Copy extra files with ''install_more'' which sould add the files
# 	to /var/opt/UNItools/ ...
# 
# 	If iptraf requires a start/stop script, then it will have to be made
...
# 
# pkg_setup ftp://iptraf.seul.org/pub/iptraf/iptraf-2.7.0.tar.gz 2.7.0 iptraf
# 
