			...
			WINDOW * win)
{
    ...
    *interval = ifr.ifr_mtu / 20;	/* There are 20 packet size brackets */
    ...
    wattrset(win, STDATTR);
    mvwprintw(win, 17, 1, "Interface MTU is %d bytes, not counting the data-link header", ifr.ifr_mtu);
    mvwprintw(win, 18, 1, "Maximum packet size is the MTU plus the data-link header length");
    mvwprintw(win, 19, 1, "Packet size computations include data-link headers, if any");
			   ...
			   int facilitytime)
{
	...
	mark_facility(PKTSIZEIDFILE, "Packet size breakdown", ifname);
        ...
        snprintf(msgstring, 80, "Packet sizes already being monitored on %s", ifname);
