...
int main(void)
{
	...
	do {
	    ss =
		select(ifd + 1, &sockset, NULL, NULL,
		       (struct timeval *) NULL);
	...
	if (FD_ISSET(cfd, &sockset)) {
	    ...
	    br =
		recvfrom(cfd, &rvnpacket, sizeof(struct rvn), 0,
	...
	if (FD_ISSET(ifd, &sockset)) {
		...
		case RVN_QUIT:
		    ...
		    unlink(IPTSOCKNAME);
		    unlink(CHILDSOCKNAME);
		    sprintf(logmsg, "rvnamed terminating: max processes spawned: %d", max_fork_count);
		    writervnlog(logfile, logmsg);
		    writervnlog(logfile, "******** rvnamed terminated ********");
		    fclose(logfile);
		    exit(0);
#endif
		case RVN_REQUEST:
		    readyidx = name_resolved(&rvnpacket, hostlist, lastfree);
		    if (readyidx >= 0) {
			...
			if (addrstat(&rvnpacket, hostlist, lastfree) ==
			    ...
			    if (fork_count <= MAX_RVNAMED_CHILDREN) {
			        ...
			        fr = fork();
                                ...
                                fr = -1;
                                if (!maxlogged)
			            writervnlog(logfile, "Maximum child process limit reached");
			        maxlogged = 1;
			    }
