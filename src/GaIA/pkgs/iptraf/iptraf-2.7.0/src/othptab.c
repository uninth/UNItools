	...
	*nomem = 1;
	...
	if (!fragment) {
		...
		new_entry->un.ospf.area =
		    ntohl(((struct ospfhdr *) packet2)->ospf_areaid.s_addr);
...
void printothpentry(struct othptable *table, struct othptabent *entry,
		    unsigned int target_row, int logging,
		    FILE * logfile)
{
	...
	writeothplog(logging, logfile, protname, "", "", 0, 0,
		     entry);
	return;
    }
    strcpy(additional, "");
    ...
    if (((entry->smacaddr)[0] != '\0') && (table->mac)) {
        snprintf(scratchpad, MSGSTRING_MAX, " (src HWaddr %s)", entry->smacaddr); 
        strcat(msgstring, scratchpad);
    }
    strcat(msgstring, " on ");
