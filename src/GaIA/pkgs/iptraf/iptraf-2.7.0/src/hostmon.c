...
void show_hostsort_keywin(WINDOW ** win, PANEL ** panel)
{
    ...
    *panel = new_panel(*win);
    ...
    wmove(*win, 4, 2);
    tx_printkeyhelp("P", " - total packets in", *win, DLGHIGHATTR, DLGTEXTATTR);
    wmove(*win, 5, 2);
    tx_printkeyhelp("I", " - IP packets in", *win, DLGHIGHATTR, DLGTEXTATTR);
    wmove(*win, 6, 2);
    tx_printkeyhelp("B", " - total bytes in", *win, DLGHIGHATTR, DLGTEXTATTR);
    wmove(*win, 7, 2);
    tx_printkeyhelp("K", " - total packets out", *win, DLGHIGHATTR, DLGTEXTATTR);
    wmove(*win, 8, 2);
    tx_printkeyhelp("O", " - IP packets out", *win, DLGHIGHATTR, DLGTEXTATTR);
    wmove(*win, 9, 2);
    tx_printkeyhelp("Y", " - total bytes out", *win, DLGHIGHATTR, DLGTEXTATTR);
    wmove(*win, 10, 2);
                           ...
                           struct ethtabent *high, int ch)
{
    ...
    if (high->index > low->index) {
        ...
        if (pivot->prev_entry != NULL)
            quicksort_lan_entries(table, low, pivot->prev_entry->prev_entry, ch);
        
        quicksort_lan_entries(table, pivot->next_entry->next_entry, high, ch);
    }
}
...
void sort_hosttab(struct ethtab *list, int *idx, int command)
{
    ...
    quicksort_lan_entries(list, list->head, list->tail->prev_entry, command);
...
void hostmon(const struct OPTIONS *options, int facilitytime, char *ifptr)
{
	...
	snprintf(msgstring, 80, "LAN station monitor already running on %s", gen_iface_msg(ifptr));
    ...
    if (logging) {
        if (strcmp(current_logfile, "") == 0)
            strncpy(current_logfile, gen_instance_logname(LANLOG, instance_id), 80);
	...
	if (br > 0) {
	    ...
	    /* Is this session for any particular interface only? */
