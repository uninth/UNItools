...
# 
#
#                                      Thu Mar 27 17:11:35 CET 2014
# Preample
# --------
# This script is the documentation for how p0f 3.06b was downloaded,
...
# All configuration files:	/var/opt/UNItools/etc/p0f
# All relevant documentation:	/var/opt/UNItools/docs/p0f.3.06b
# 
...
# cd /lan/ssi/shared/software/internal/UNItools/src/GaIA/pkgs/p0f
# 
#  1	Download the package p0f, and read the package documentation,
# 	usually README and INSTALL.
# 
# wget -N http://lcamtuf.coredump.cx/p0f3/releases/p0f-3.06b.tgz
# 
# /bin/rm -fr p0f-3.06b
# /bin/gtar xvzf p0f-3.06b.tgz
# 
...
VENDOR="http://lcamtuf.coredump.cx/p0f3/releases/p0f-3.06b.tgz"
...
export LDFLAGS=-static
	...
	# fix problems with old compiler and macro preprocessor directives
	if [ ! -f p0f.c.dist ]; then
		cp p0f.c p0f.c.dist
	fi

	sed 's/ERRORF(/fprintf(stderr,/' p0f.c.dist > p0f.c
	diff p0f.c.dist p0f.c
	
	# prepare for installation
	if [ ! -f config.h.dist ]; then
		cp config.h config.h.dist
	fi
	ls -l config.h config.h.dist
	sed '
		s%#.*define.*FP_FILE.*%#  define FP_FILE           "/var/opt/UNItools/etc/p0f.fp"%' \
...
# 
# 	If p0f is made with automake and configure then
# 

 	pkg_conbat -n ./CFG

#	But it's not, so the installation procedure is done by install_more.sh
#	and the tools has to be compiled separately as well.
	(
	. ./CFG
	cd $SRC/tools
	make
...
# 	/var/opt/UNItools
# 	If p0f requires something else to compile, please correct the
# 	section that writes to CFG.
# 	The new files are then moved to p0f.3.06b.install.tmp - which
# 	will be wiped first.
# 	Copy extra files with ''install_more'' which sould add the files
# 	to /var/opt/UNItools/ ...
# 
# 	If p0f requires a start/stop script, then it will have to be made
...
# 
# pkg_setup http://lcamtuf.coredump.cx/p0f3/releases/p0f-3.06b.tgz 3.06b p0f
# 
