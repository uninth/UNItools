...
*/
char 	*prcline   	= "PRC | sys %s | user %s | #thr %s | #zombie %s |"
	       	     	  " #exit %s |\n";
/***************************************************************************/
char	*genprochdr1	= "\n  PID  SYSCPU  USRCPU  VGROW  RGROW USERNAME THR  "
		     	  "ST EXC  S  %s CMD  %4d/%-4d\n";
...
char	*memprochdr	= "\n  PID MINFLT MAJFLT      VSTEXT "
		     	  " VSIZE  RSIZE  VGROW  RGROW  %s CMD  %4d/%-4d\n";
...
char	*totuserhdr	= "\nNPROCS  SYSCPU  USRCPU  VSIZE  RSIZE  RDDSK "
			  "WRDSK RNET SNET  %s USER %4d/%-4d\n";
char 	*totuserlin	= "%s %s %s %s %s  %s %s %s %s "
	                  "%3.0lf%% %.14s\n";
/***************************************************************************/
char	*totprochdr	= "\nNPROCS  SYSCPU  USRCPU  VSIZE  RSIZE  RDDSK "
			  "WRDSK RNET SNET  %s CMD  %4d/%-4d\n";
                                ...
                                int avgval, int nsecs)
{
	...
	printg(prcline, val2cpustr(totst * 1000/HZ, format1),
			...
			val2valstr(nproc,           format3, 7, 0, 0),
...
showvarproc(struct pstat *curstat, double perc, int nsecs, int avgval)
{
	...
	*/
	...
	if ( (grp = getgrgid(curstat->gen.rgid)) == NULL)
		groupname = "unknown";
	else
...
*/
void
priphead(int curlist, int totlist, char *sortorder, char showtype)
{
	switch (showtype)
	{
	   case MPROCGEN:
		if ( !(kernpatch & PATCHSTAT) )
			printg(genprochdr1, sortorder, curlist, totlist);
		else
			printg(genprochdr2, sortorder, curlist, totlist);
		break;

	   case MPROCMEM:
		printg(memprochdr, sortorder, curlist, totlist);
		break;

	   case MPROCDSK:
		printg(dskprochdr, sortorder, curlist, totlist);
		break;

	   case MPROCNET:
		printg(netprochdr, sortorder, curlist, totlist);
		break;

	   case MPROCVAR:
		printg(varprochdr, sortorder, curlist, totlist);
		break;

	   case MPROCARG:
		printg(cmdprochdr, sortorder, curlist, totlist);
		break;

	   case MPROCSCH:
		printg(schprochdr, sortorder, curlist, totlist);
		break;

	   case MCUMUSER:
		printg(totuserhdr, sortorder, curlist, totlist);
		break;

	   case MCUMPROC:
		printg(totprochdr, sortorder, curlist, totlist);
		break;
	}
}
...
priproc(struct pstat *pstat, int firstproc, int lastproc, int curline,
	int curlist, int totlist, char *sortorder,
	struct syscap *sb, char showtype,
	struct selection *sel, int screen, int nsecs, int avgval)
{
	register int		i;
...
*/
int
prisyst(struct sstat *sstat, int curline, int nsecs, int avgval, int fixedhead, 
		int maxcpulines, int maxdsklines, int maxintlines)
{
	register int	i, lin;
	count_t		cputot, percputot, mstot;
	char		format1[16], format2[16], format3[16];
	char		format4[16], format5[16];
	...
	** MEMORY statistics
	*/
	printg("MEM | tot  %s | free  %s | cache %s | buff  %s | slab  %s |\n",
	...
	*/
	...
	for (i=0, lin=0; sstat->xdsk.xdsk[i].name[0] && lin < maxdsklines; i++)
	{
		...
		if (iotot || fixedhead)
		{
		    printg("DSK | %11.11s | busy %6.0lf%% | read %s |"
	...
	*/
	...
	for (i=0, lin=0; sstat->intf.intf[i].name[0] && lin < maxintlines; i++)
	{
		...
		{
			count_t ival   = sstat->intf.intf[i].rbyte/128/nsecs;
			count_t oval   = sstat->intf.intf[i].sbyte/128/nsecs;
			...
			printg("NET | dev %7.7s | pcki %s | pcko %s |"
			       " in %4lld %s | out %3lld %s |\n",
