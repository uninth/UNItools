...
#define	allflags	"ab:cde:fghijklmnopqrstuvwxyz1ABCDEFGHIJKLMNOPQRSTUVWXYZ"
#define	PROCCHUNK	100	/* portion-size for process-table expansion */
...
** timing info
*/
time_t 		curtime;	/* might be used external by other functions */
time_t 		pretime;	/* might be used external by other functions */

static char	awaittrigger;	/* boolean: awaiting trigger for next cycle  */
...
/*
** next struct contains pointers to four visualization-functions:
**   1.	show all info about one sample
**   2.	show error-message
**   3.	handle end-of-program
**   4.	show usage-info
*/
...
main(int argc, char *argv[])
{
	...
	if ( (p = getenv("HOME")) )
	{
		...
		*/
		if ( access(path, R_OK) == 0)
		{
			...
			{
				if ( sscanf(linebuf, "%15s %15s",
							tagname, tagvalue) < 2)
					continue;

				if (tagname[0] == '#' || tagvalue[0] == '#')
					continue;
	...
	*/
	...
	/*
	** determine time since when current statistics have
	** been maintained
	*/
	curtime = getboot();
	...
	*/
	...
	/*
	** since process-accounting is switched on now, there is no
...
engine(void)
{
	...
	*/
	...
	if (cursstat == NULL || presstat == NULL || devsstat == NULL)
	{
