...
# Konfiguration: /var/opt/UNItools/etc/di
# Dokumentation: /var/opt/UNItools/docs/di.4.9
# 
# Det er *ikke* programpakker i Linux format (pkg) men blot en
# installation af programmet di.
# 
#  1	Download programpakken di til /lan/ssi/projects/UNItools/src/Linux/2.4.21-20cpsmp/pkgs/di.
...
# 
#	Dette er ikke den helt rigtige måde at bygge programmet på (se README)
#	men det falder mere i tråd med den måde pakke-bygge-systemer virker på.
#	Erstat /usr/local (prefix) med /var/opt/UNItools i Makefile

	if [ ! -f di-4.9/Makefile.dist ]; then
		cp di-4.9/Makefile di-4.9/Makefile.dist
	fi

	sed "
		s%prefix.*=.*%prefix = /var/opt/UNItools%
	" di-4.9/Makefile.dist > di-4.9/Makefile
#
...
# 	som html, txt og pdf og gemmes i docs/di.4.9.  Se efter
...
# 	kopiere de relevante dokumenter til di.4.9.install.tmp.
# 
