#! /bin/sh
#
# Test script for dialog
#
ERROR=0
TMPROOT=/lan/ssi/shared/software/internal/UNItools/src/GaIA/pkgs/dialog/dialog.1.0-20051107.install.tmp

function rmfiles()
{
	echo "removing files ..."
/bin/rm -f /var/opt/UNItools/docs/dialog.1.0-20051107/samples/.info
}

function test_noclobber()
{
	echo "testing that installation will not clobber existing files"
if [ -f /var/opt/UNItools/docs/dialog.1.0-20051107/samples/.info ]; then
	echo "ERROR: file /var/opt/UNItools/docs/dialog.1.0-20051107/samples/.info exists"
	ERROR=1
fi
}
