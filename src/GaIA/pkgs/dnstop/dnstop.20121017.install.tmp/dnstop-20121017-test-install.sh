#! /bin/sh
#
# Test script for dnstop
#
ERROR=0
TMPROOT=/lan/ssi/shared/software/internal/UNItools/src/GaIA/pkgs/dnstop/dnstop.20121017.install.tmp

function rmfiles()
{
	echo "removing files ..."
/bin/rm -f /var/opt/UNItools/docs/dnstop.20121017/LICENSE
...
if [ -f /var/opt/UNItools/docs/dnstop.20121017/LICENSE ]; then
	echo "ERROR: file /var/opt/UNItools/docs/dnstop.20121017/LICENSE exists"
	ERROR=1
fi

if [ -f /var/opt/UNItools/bin/dnstop ]; then
	echo "ERROR: file /var/opt/UNItools/bin/dnstop exists"
	ERROR=1
fi
}
