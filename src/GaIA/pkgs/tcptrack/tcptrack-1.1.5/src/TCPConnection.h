...
public:
	...
	TCPConnection( IPv4TCPCapture &p );
	~TCPConnection();

	// returns true if the given addresses/ports are relevant to this
	// connection.
	bool match(IPv4Address &sa, IPv4Address &da, portnum_t sp, portnum_t dp);
	...
	//bool acceptPacket( IPv4Packet *p );
	bool acceptPacket( IPv4TCPCapture &p );
...
private:
	...
	void fastRecalcAvg();
	void slowRecalcAvg();
	void updateCountersForPacket( IPv4TCPCapture &p );
	...
	IPv4Address *srcaddr; // client addr
	IPv4Address *dstaddr; // server addr
