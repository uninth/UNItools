...
void generateIOSReport(struct nipperConfig *nipper)
{
	...
	if (nipper->fullReport == true)
	{
		...
		while (interfaceConfigPointer != 0)
		{
			if ((interfaceConfigPointer->mop == true) && (interfaceConfigPointer->shutdown == false) && (strcmp(interfaceConfigPointer->ipAddress, "None") != 0) && (strncmp(interfaceConfigPointer->name, "Loopback", 8) != 0))
			{
				tempInt++;
			}
			interfaceConfigPointer = interfaceConfigPointer->next;
