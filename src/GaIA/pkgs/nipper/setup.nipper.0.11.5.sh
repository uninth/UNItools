...
# 
#
#                                      Fri Mar 18 17:23:26 CET 2016
# Preample
# --------
# This script is the documentation for how nipper 0.11.5 was downloaded,
...
# All configuration files:	/var/opt/UNItools/etc/nipper
# All relevant documentation:	/var/opt/UNItools/docs/nipper.0.11.5
# 
...
# cd /lan/ssi/shared/software/internal/UNItools/src/GaIA/pkgs/nipper
# 
#  1	Download the package nipper, and read the package documentation,
# 	usually README and INSTALL.
# 
#wget -N http://www.darknet.org.uk/content/files/nipper-0.11.5.tgz
# 
#/bin/gtar xvzf nipper-0.11.5.tgz
# 
...
VENDOR="http://www.darknet.org.uk/content/files/nipper-0.11.5.tgz"
...
# 
# 	If nipper is made with automake and configure then
# 
#	Well, it's not.
	...
	sed '
		s%^BINPATH.*%BINPATH = /var/opt/UNItools/bin%
		s%^CONFPATH.*%CONFPATH = /var/opt/UNItools/etc/%
		s%^MANPATH.*%MANPATH = /var/opt/UNItools/share/man/%
		' Makefile.org > Makefile

	test -f common/nipper-config.c.org || {
		cp common/nipper-config.c common/nipper-config.c.org
	}
	test -f common/nipper-help.c.org || {
		cp common/nipper-help.c common/nipper-help.c.org
	}

	sed 's%/etc/nipper.conf%/var/opt/UNItools/etc/nipper.conf%g' common/nipper-config.c.org > common/nipper-config.c
	sed 's%/etc/nipper.conf%/var/opt/UNItools/etc/nipper.conf%g' common/nipper-help.c.org > common/nipper-help.c
...
# 	/var/opt/UNItools
# 	If nipper requires something else to compile, please correct the
# 	section that writes to CFG.
# 	The new files are then moved to nipper.0.11.5.install.tmp - which
# 	will be wiped first.
# 	Copy extra files with ''install_more'' which sould add the files
# 	to /var/opt/UNItools/ ...
# 
# 	If nipper requires a start/stop script, then it will have to be made
...
# 
# pkg_setup http://www.darknet.org.uk/content/files/nipper-0.11.5.tgz 0.11.5 nipper
# 
