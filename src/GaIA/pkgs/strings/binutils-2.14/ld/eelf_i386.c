...
static void gldelf_i386_before_parse
  PARAMS ((void));
static void gldelf_i386_vercheck
  PARAMS ((lang_input_statement_type *));
static void gldelf_i386_stat_needed
  PARAMS ((lang_input_statement_type *));
static bfd_boolean gldelf_i386_try_needed
  PARAMS ((const char *, int));
static bfd_boolean gldelf_i386_search_needed
  PARAMS ((const char *, const char *, int));
static void gldelf_i386_check_needed
  PARAMS ((lang_input_statement_type *));
static void gldelf_i386_after_open
  PARAMS ((void));
static void gldelf_i386_find_exp_assignment
  PARAMS ((etree_type *));
static void gldelf_i386_find_statement_assignment
  PARAMS ((lang_statement_union_type *));
static void gldelf_i386_before_allocation
  PARAMS ((void));
static bfd_boolean gldelf_i386_open_dynamic_archive
...
static void gldelf_i386_finish
  PARAMS ((void));
static char *gldelf_i386_get_script
  PARAMS ((int *isfile));
...
static bfd_boolean
gldelf_i386_try_needed (name, force)
     const char *name;
     int force;
{
  bfd *abfd;
...
static bfd_boolean
gldelf_i386_search_needed (path, name, force)
     const char *path;
     const char *name;
     int force;
{
  ...
  if (name[0] == '/')
    return gldelf_i386_try_needed (name, force);
  ...
  while (1)
      ...
      else
	{
      ...
      if (gldelf_i386_try_needed (filename, force))
	return TRUE;
...
static char * gldelf_i386_add_sysroot PARAMS ((const char *));

static char *
gldelf_i386_add_sysroot (path)
     const char *path;
{
  int len, colons, i;
  char *ret, *p;
...
static bfd_boolean gldelf_i386_check_ld_so_conf
  PARAMS ((const char *, int));

static bfd_boolean
gldelf_i386_check_ld_so_conf (name, force)
     const char *name;
     int force;
{
  ...
  if (! initialized)
      ...
      if (f != NULL)
	{
	  ...
	  if (b)
	    {
	      char *d = gldelf_i386_add_sysroot (b);
	      free (b);
	      b = d;
	    }
  ...
  return gldelf_i386_search_needed (ld_so_conf, name, force);
}
      ...
      for (force = 0; force < 2; force++)
	{
	  ...
	  if (command_line.rpath_link == NULL
	      && command_line.rpath == NULL)
	    {
	      ...
	      if (gldelf_i386_search_needed (lib_path, l->name,
						      force))
		break;
	    }
	  lib_path = (const char *) getenv ("LD_LIBRARY_PATH");
	  if (gldelf_i386_search_needed (lib_path, l->name, force))
	    break;
	  found = 0;
...
static void
gldelf_i386_find_exp_assignment (exp)
     etree_type *exp;
{
...
static void
gldelf_i386_find_statement_assignment (s)
...
static bfd_boolean
gldelf_i386_open_dynamic_archive (arch, search, entry)
     const char *arch;
     search_dirs_type *search;
...
static bfd_boolean
gldelf_i386_place_orphan (file, s)
...
static void gldelf_i386_add_options
  PARAMS ((int, char **, int, struct option **, int, struct option **));

static void
gldelf_i386_add_options (ns, shortopts, nl, longopts, nrl, really_longopts)
     int ns;
     char **shortopts;
     int nl;
...
static bfd_boolean gldelf_i386_handle_option
  PARAMS ((int));

static bfd_boolean
gldelf_i386_handle_option (optc)
     int optc;
{
  switch (optc)
    {
...
static void gldelf_i386_list_options PARAMS ((FILE * file));

static void
gldelf_i386_list_options (file)
     FILE * file;
{
