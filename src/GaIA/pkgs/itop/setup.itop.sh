...
# Konfiguration: /var/opt/UNItools/etc/itop
# Dokumentation: /var/opt/UNItools/docs/itop.0.1.1
# 
# Det er *ikke* programpakker i Linux format (pkg) men blot en
# installation af programmet itop.
# 
#  1	Download programpakken itop til /lan/ssi/projects/UNItools/src/Linux/2.4.21-20cpsmp/pkgs/itop.
# 	Læs hvordan programpakken oversættes og installeres i dokumentationen
# 	for programpakken (typisk README og INSTALL).
# 
# 	wget -N http://freshmeat.net/redir/itop/17843/url_bz2/itop-0.1.tar.bz2
# 
	bunzip2 -c itop-0.1.tar.bz2 > itop-0.1.tar
 	/bin/gtar xvf itop-0.1.tar
# 
...
# 
#	itop har ikke en configure, derfor rettes Makefile on-the-fly

	if [ ! -f itop-0.1/src/Makefile.dist  ]; then
		cp itop-0.1/src/Makefile itop-0.1/src/Makefile.dist
	fi

	cat << 'EOF' | sed "s/_DATE_/`date`/g" > itop-0.1/Makefile
#
# fakemake -- src is in src _DATE_
#
CC = gcc -Wall -static
...
install: itop
	/usr/local/bin/install-sh -c -m 555 src/itop /var/opt/UNItools/bin/itop
	/usr/local/bin/srctoman - ../itop.srctoman > /var/opt/UNItools/share/man/man1/itop.1
EOF
...
# 	som html, txt og pdf og gemmes i docs/itop.0.1.1.  Se efter
...
# 	kopiere de relevante dokumenter til itop.0.1.1.install.tmp.
# 
