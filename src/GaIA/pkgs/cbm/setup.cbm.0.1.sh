...
# 
#
#                                      Wed Mar 19 12:41:06 CET 2014
# Preample
# --------
# This script is the documentation for how cbm 0.1 was downloaded,
...
# All configuration files:	/var/opt/UNItools/etc/cbm
# All relevant documentation:	/var/opt/UNItools/docs/cbm.0.1
# 
...
# cd /lan/ssi/shared/software/internal/UNItools/src/GaIA/pkgs/cbm
# 
#  1	Download the package cbm, and read the package documentation,
# 	usually README and INSTALL.
# 
# wget -N http://www.isotton.com/utils/cbm/download/cbm-0.1.tar.gz
# 
# /bin/gtar xvzf cbm-0.1.tar.gz
# 
...
URL="http://www.isotton.com/utils/cbm/download/cbm-0.1.tar.gz"
PKG="cbm"
VER="0.1"
...
VENDOR="http://www.isotton.com/utils/cbm/download/cbm-0.1.tar.gz"
...
# 
# 	#include <unistd.h> has to be added to the top of cbm.cpp. So replace 
#	the first blank line with it:

(
	cd cbm-0.1/src
	if [ ! -f cbm.cpp.org ]; then
		cp cbm.cpp cbm.cpp.org
	fi
	sed '0,/^$/{s//#include <unistd.h>/}' cbm.cpp.org > cbm.cpp
	diff cbm.cpp cbm.cpp.org
)

# 	If cbm is made with automake and configure then
#
 	pkg_conbat -n ./CFG
# 
...
# 	/var/opt/UNItools
# 	If cbm requires something else to compile, please correct the
# 	section that writes to CFG.
# 	The new files are then moved to cbm.0.1.install.tmp - which
# 	will be wiped first.
# 	Copy extra files with ''install_more'' which sould add the files
# 	to /var/opt/UNItools/ ...
# 
# 	If cbm requires a start/stop script, then it will have to be made
...
# 
# pkg_setup http://www.isotton.com/utils/cbm/download/cbm-0.1.tar.gz 0.1 cbm
# 
