...
use constant AIX         => 'aix'     eq $^O;
use constant SOLARIS     => 'solaris' eq $^O;
use constant SOLARIS_X86 => SOLARIS   && $Config{archname} =~ /i86pc|x86/;
use constant WIN32       => 'MSWin32' eq $^O;
...
#=============================================================================
use constant DELIM	=> WIN32 ? ';' : ':';
use constant PATHSEP	=> WIN32 ? '\\' : '/';
...
sub repository {
    ...
    #No ppm repositories for these platforms
    return if SOLARIS_X86 or AIX;
...
sub config_file_stub {
	...
	my $tmp = $ENV{TEMP} || WIN32 ? 'C:\TEMP' : '/tmp';
...
END
	...
	my $repositories;
	if (my $url_ppm3 = repository('ppm3')) {
		$repositories .= <<END;
